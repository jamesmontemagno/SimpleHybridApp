...
With no `-Platform` argument, script `04` asks where to launch: Windows or Android on Windows; Mac Catalyst, iOS, or Android on macOS. Pass `-Platform Windows`, `-Platform Android`, `-Platform iOS`, or `-Platform MacCatalyst` to skip the prompt. For Android, the script selects the only connected target, starts the DevFlow broker, and configures the required ADB reverse and forward mappings before launch. If multiple Android targets are connected, pass `-Device <identifier>` using an identifier shown by `03-devices.ps1 -Action List`.